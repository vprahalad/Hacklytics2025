  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/send", {
        ...
        body: JSON.stringify({ message }), // Send user input as JSON
      });

      const data = await response.json();
      console.log("Response from backend:", data);
      setOutput(data.response || "No response"); // update output from backend encoding
      setMetric(data.metric || 0); // set metric from backend
      setKeyData(data.key || null); // store JSON key file from backend
      setShowOutput(true);
    } catch (error) {
  ...
  return (
    <div className="container">
      ...
      <div className="left-panel">
        ...
        <img src="/logo4.png" alt="Logo" className="w-40 h-auto mb-4" />
        ...
        <div className="input-container">
          ...
          {showOutput && (
            <div className="progress-bar-container">
              ...
              <p className="cipher-strength-label">Cipher Strength</p>
            </div>
          )}
